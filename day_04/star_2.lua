...
local function can_lift_paper_roll(i, j, map)
...
local coordinates_to_be_removed = {}
while true do
    for i, row in ipairs(map) do
        ...
        for j, char in ipairs(row) do
            line = line .. char
            if char == '@' then
                local res = can_lift_paper_roll(i, j, map)
                if res then
                    can_be_lifted_count = can_be_lifted_count + 1
                    local coordinates = {i, j}
                    table.insert(coordinates_to_be_removed, coordinates)
                end
            end
        end
        -- print(line)
    ...
    if #coordinates_to_be_removed == 0 then
        goto out
    end

    for i, coordinates in ipairs(coordinates_to_be_removed) do
        map[coordinates[1]][coordinates[2]] = 'x'
    end

    coordinates_to_be_removed = {}
